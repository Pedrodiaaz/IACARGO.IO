...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_aud:
        ...
        busq_aud = st.text_input("🔍 Buscar paquete por ID de Guía en el historial:")
        if st.session_state.inventario:
    ...
    with t_res:
        ...
        if st.session_state.inventario:
            ...
            # Secciones por Estado (Para tu análisis)
            ...
            for est, ico in estados:
                ...
                else: st.info(f"No hay paquetes en esta fase.")
...
# --- 5. PANEL DEL CLIENTE ---
    ...
    if mis_p:
        for p in mis_p:
            with st.container():
                st.markdown(f'<div class="p-card"><h3>Guía: {p.get("ID_Barra","N/A")}</h3><p>Estado: <b>{p.get("Estado","Procesando")}</b></p></div>', unsafe_allow_html=True)
