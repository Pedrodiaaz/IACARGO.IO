...
st.markdown("""
    ...
    /* --- AJUSTE SOLICITADO PARA EXPANDERS (COBROS Y OTROS) --- */
    /* Mantenemos el fondo azul y letras blancas SIEMPRE, abierto o cerrado */
    [data-testid="stExpander"] {
        background: #2563eb !important; /* Azul primario fijo */
    ...
    /* El interior del expander cuando se abre */
...
def render_admin_dashboard():
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            if st.form_submit_button("REGISTRAR EN SISTEMA", type="primary", use_container_width=True):
                if f_id and f_cli and f_cor:
                    ...
                    guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.session_state.id_actual = generar_id_unico(); agregar_notificacion(f"Registro: {f_id}"); st.rerun()
    ...
    with t_res:
        ...
        for i, (est, label) in enumerate([("RECIBIDO ALMACEN PRINCIPAL", "EN ALMACÉN"), ("EN TRANSITO", "EN TRÁNSITO"), ("ENTREGADO", "ENTREGADO")]):
            count = len(df_res[df_res['Estado'] == est]) if not df_res.empty else 0
            [m1, m2, m3][i].markdown(f'<div class="metric-container"><small>{label}</small><br><b style="font-size:25px;">{count}</b></div>', unsafe_allow_html=True)
        
        busq_box = st.text_input(" Buscar caja:", key="res_search_admin")
