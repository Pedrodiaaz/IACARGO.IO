...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_reg:
        ...
        f_tra = st.selectbox("Tipo de Traslado", ["Aéreo", "Marítimo"], key="reg_traslado")
    ...
    with t_val:
        ...
        if pendientes:
            guia_v = st.selectbox("Seleccione Guía:", [p["ID_Barra"] for p in pendientes], key="val_guia_sel")
            paq = next(p for p in pendientes if p["ID_Barra"] == guia_v)
            peso_real = st.number_input("Medida Real", min_value=0.0, value=float(paq['Peso_Mensajero']), key="val_peso_input")
            if st.button("⚖️ Confirmar", key="val_confirm_btn"):
    ...
    with t_cob:
        ...
        for p in pendientes_pago:
            # SOLUCIÓN QUIRÚRGICA: Añadimos prefijo 'cob_' a las llaves
            with st.expander(f"💵 {p['ID_Barra']} - {p['Cliente']}"):
                resta = p['Monto_USD'] - p.get('Abonado', 0.0)
                st.write(f"Restante: **${resta:.2f}**")
                monto_abono = st.number_input(f"Abonar a {p['ID_Barra']}", 0.0, float(resta), key=f"cob_input_{p['ID_Barra']}")
    ...
    with t_est:
        ...
        if st.session_state.inventario:
            sel_e = st.selectbox("ID de Guía:", [p["ID_Barra"] for p in st.session_state.inventario], key="est_guia_sel")
            n_st = st.selectbox("Nuevo Estado:", ["RECIBIDO ALMACEN PRINCIPAL", "EN TRANSITO", "ENTREGADO"], key="est_new_val")
            if st.button("Actualizar", key="est_update_btn"):
    ...
    with t_aud:
        ...
        # Para evitar choques en auditoría, las llaves de edición también llevan prefijo
        if st.session_state.inventario:
            guia_ed = st.selectbox("Editar ID:", [p["ID_Barra"] for p in st.session_state.inventario], key="aud_guia_sel")
            paq_ed = next((p for p in st.session_state.inventario if p["ID_Barra"] == guia_ed), None)
            if paq_ed:
                c1, c2 = st.columns(2)
                with c1: n_cli = st.text_input("Cliente", value=paq_ed['Cliente'], key=f"aud_cli_{paq_ed['ID_Barra']}")
                with c2: n_pes = st.number_input("Peso", value=float(paq_ed['Peso_Almacen']), key=f"aud_pes_{paq_ed['ID_Barra']}")
                if st.button("💾 Guardar Cambios", key=f"aud_save_{paq_ed['ID_Barra']}"):
                    paq_ed.update({'Cliente': n_cli, 'Peso_Almacen': n_pes})
                    guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()

    with t_res:
        st.subheader("📊 Resumen General")
        if st.session_state.inventario:
            busq_res = st.text_input("🔍 Buscar caja por ID:", key="res_search_input")
            import pandas as pd
            df_res = pd.DataFrame(st.session_state.inventario)
            if busq_res: df_res = df_res[df_res['ID_Barra'].astype(str).str.contains(busq_res, case=False)]
            
            # Mostrar por estados con expansores para no saturar
            for est_k, est_l in [("RECIBIDO ALMACEN PRINCIPAL", "📦 Almacén"), ("EN TRANSITO", "✈️ Tránsito"), ("ENTREGADO", "✅ Entregado")]:
                ...
                with st.expander(f"{est_l} ({len(df_f)})", expanded=True):
...
# --- 5. PANEL DEL CLIENTE ---
    ...
    # Buscador para el cliente con su propia key
    busq_cli = st.text_input("🔍 Buscar mis paquetes:", key="cli_search_box")
    ...
    if not mis_p: st.info("No hay paquetes.")
        ...
        for i, p in enumerate(mis_p):
            with (col_p1 if i % 2 == 0 else col_p2):
                ...
                st.markdown(f'<div class="p-card"><b>ID: {p["ID_Barra"]}</b><br>Estado: {p["Estado"]}</div>', unsafe_allow_html=True)
...
# --- 6. LOGIN ---
    ...
    with c2:
        ...
        t_l, t_r = st.tabs(["Ingresar", "Registrarse"])
        with t_l:
            le = st.text_input("Correo", key="login_mail")
