...
def agregar_notificacion(mensaje, destino="admin", correo_cliente=None):
    """Agrega notificaciones segmentadas por rol"""
    ...
    with col_n:
        ...
        with st.popover("🔔"):
            ...
            for n in mis_notif:
                ...
                n['leida'] = True
            if mis_notif and st.button("Limpiar Notificaciones"):
...
def render_admin_dashboard():
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            if st.form_submit_button("REGISTRAR EN SISTEMA", type="primary"):
                if f_id and f_cli and f_cor:
                    ...
                    agregar_notificacion(f"📦 Nuevo paquete registrado: {f_id} para {f_cli}", "admin")
    ...
    with t_val:
        ...
        if pendientes:
            ...
            v_real = st.number_input("Valor Real", min_value=0.0, value=float(paq['Peso_Mensajero']))
            if st.button("CONFIRMAR Y VALIDAR", type="primary"):
                ...
                agregar_notificacion(f"✅ Medida verificada para {paq['ID_Barra']}: {v_real} {unidad}", "cliente", paq['Correo'])
                
                guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()

    with t_cob:
        st.subheader("Gestión de Cobros")
        busq_cob = st.text_input("🔍 Buscar paquete o cliente:", key="search_cobros_admin")
        ...
        for p in pendientes_p:
            ...
            with st.expander(f"📦 {p['ID_Barra']} — {p['Cliente']} (Faltan: ${rest:.2f})"):
                ...
                if st.button(f"REGISTRAR PAGO", key=f"bp_{p['ID_Barra']}", type="primary"):
                    ...
                    agregar_notificacion(f"💰 Pago registrado: ${m_abono:.2f} para la guía {p['ID_Barra']}", "cliente", p['Correo'])
                    guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()
    ...
    with t_est:
        ...
        if st.button("ACTUALIZAR ESTATUS", type="primary"):
            for p in st.session_state.inventario:
                if p["ID_Barra"] == sel_e: 
                    p["Estado"] = n_st
                    agregar_notificacion(f"🚚 Movimiento: Tu paquete {p['ID_Barra']} ahora está {n_st}", "cliente", p['Correo'])
            guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()
        ...
        for est_k, est_l in [("RECIBIDO ALMACEN PRINCIPAL", "📦 EN ALMACÉN"), ("EN TRANSITO", "🚚 EN TRÁNSITO"), ("ENTREGADO", "✅ ENTREGADO")]:
            ...
            with st.expander(f"{est_l} ({len(df_f)})"):
                for _, r in df_f.iterrows():
                    ...
                    st.markdown(f'<div class="resumen-row"><div style="color:#2563eb; font-weight:800;">{icon} {r["ID_Barra"]}</div><div style="flex-grow:1; margin-left:15px;">{r["Cliente"]}</div><div style="font-weight:700;">${float(r["Abonado"]):.2f}</div></div>', unsafe_allow_html=True)
            ...
            with (c1 if i % 2 == 0 else c2):
                ...
                <div class="p-card">
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 10px; font-size: 14px;">
                        ...
                        <span style="color:{'#4ade80' if p['Validado'] else '#facc15'}">{'● Peso Verificado' if p['Validado'] else '○ Pendiente Validación'}</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
