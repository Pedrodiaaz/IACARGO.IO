...
def render_admin_dashboard():
    ...
    # (Lógica de Registro y Validación se mantienen igual que tu código original)
    with t_reg:
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            if st.form_submit_button("REGISTRAR", type="primary"):
                ...
                nuevo = {"ID_Barra": f_id, "Cliente": f_cli, "Correo": f_cor.lower().strip(), "Peso_Mensajero": f_pes, "Peso_Almacen": 0.0, "Validado": False, "Monto_USD": monto, "Estado": "RECIBIDO ALMACEN PRINCIPAL", "Pago": "PENDIENTE", "Modalidad": f_mod, "Tipo_Traslado": f_tra, "Abonado": 0.0, "Fecha_Registro": datetime.now()}
                st.session_state.inventario.append(nuevo)
                guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()

    with t_val:
        st.subheader(" Validación en Almacén")
        p_val = [p for p in st.session_state.inventario if not p.get('Validado')]
        if p_val:
            guia_v = st.selectbox("Guía:", [p["ID_Barra"] for p in p_val])
            paq = next(p for p in p_val if p["ID_Barra"] == guia_v)
            v_real = st.number_input("Valor Real Almacén", min_value=0.0, value=float(paq['Peso_Mensajero']))
    ...
    with t_cob:
        ...
        # --- NUEVO: BUSCADOR DE COBROS ---
        busq_cobro = st.text_input("🔍 Ubicar cobro (ID o Cliente):", key="search_cobro_admin")
        
        pendientes_p = [p for p in st.session_state.inventario if p['Pago'] == 'PENDIENTE']
        
        if busq_cobro:
            pendientes_p = [p for p in pendientes_p if busq_cobro.lower() in str(p['ID_Barra']).lower() or busq_cobro.lower() in str(p['Cliente']).lower()]
    ...
    # (Las demás pestañas: t_est, t_aud, t_res se mantienen exactamente como en tu código original)
    with t_est:
        st.subheader(" Estatus de Logística")
        if st.session_state.inventario:
            sel_e = st.selectbox("Guía:", [p["ID_Barra"] for p in st.session_state.inventario], key="s_e")
            n_st = st.selectbox("Estatus:", ["RECIBIDO ALMACEN PRINCIPAL", "EN TRANSITO", "RECIBIDO EN ALMACEN DE DESTINO", "ENTREGADO"])
...
# --- DASHBOARD CLIENTE (CON BUSCADOR) ---
def render_client_dashboard():
    u = st.session_state.usuario_identificado
    st.markdown(f'<div class="welcome-text">Bienvenido, {u["nombre"]}</div>', unsafe_allow_html=True)
    
    # --- NUEVO: BUSCADOR DE PAQUETES ---
    busq_cliente = st.text_input("🔍 Ubicar paquete (ID o Estado):", key="search_client_main")
    
    mis_p = [p for p in st.session_state.inventario if str(p.get('Correo', '')).lower() == str(u.get('correo', '')).lower()]
    
    if busq_cliente:
        mis_p = [p for p in mis_p if busq_cliente.lower() in str(p['ID_Barra']).lower() or busq_cliente.lower() in str(p['Estado']).lower()]

    if not mis_p: 
        st.info("Sin envíos que coincidan con la búsqueda.")
    else:
        c1, c2 = st.columns(2)
...
# --- HEADER Y LÓGICA DE NAVEGACIÓN (Se mantiene igual) ---
def render_header():
...
if st.session_state.usuario_identificado is None:
    if st.session_state.landing_vista:
        ...
        if st.button("INGRESAR", use_container_width=True, type="primary"): st.session_state.landing_vista = False; st.rerun()
    else:
        # Aquí iría tu lógica de login/registro original...
        c1, c2, c3 = st.columns([1, 1.5, 1])
        with c2:
            st.markdown('<div class="logo-animado" style="font-size:60px; width:100%; text-align:center;">IACargo.io</div>', unsafe_allow_html=True)
            ...
            with t2:
                st.info("Formulario de registro activo")
