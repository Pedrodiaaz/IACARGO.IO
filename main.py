...
st.markdown("""
    ...
    .bell-container { position: relative; display: inline-block; cursor: pointer; }
    .bell-icon { font-size: 26px; color: #facc15; }
    .red-dot {
        position: absolute; top: -2px; right: -2px;
        height: 12px; width: 12px;
        background-color: #ef4444; border-radius: 50%;
        border: 2px solid #0f172a; z-index: 10;
    }
    ...
    div[data-testid="stPopoverContent"] p, div[data-testid="stPopoverContent"] b, div[data-testid="stPopoverContent"] small {
...
def agregar_notificacion(mensaje, destino="admin", correo_cliente=None):
    """
    destino: 'admin', 'cliente' o 'todos'
    correo_cliente: Necesario si el destino es 'cliente'
    """
...
def render_header():
    ...
    with col_n:
        ...
        if u['rol'] == 'admin':
            mis_notif = [n for n in st.session_state.notificaciones if n['destino'] in ['admin', 'todos']]
        else:
            mis_notif = [n for n in st.session_state.notificaciones if n['destino'] == 'cliente' and n['correo'] == u['correo']]
        
        has_unread = any(not n['leida'] for n in mis_notif)
        
        dot_html = '<div class="red-dot"></div>' if has_unread else ""
        with st.popover("🔔"):
            st.markdown(f'<div class="bell-container"><span class="bell-icon">🔔</span>{dot_html}</div>', unsafe_allow_html=True)
            st.write("---")
            if not mis_notif:
                st.small("No hay novedades por ahora.")
            for n in mis_notif:
                st.markdown(f'<div class="notif-item"><b>{n["hora"]}</b> - {n["msg"]}</div>', unsafe_allow_html=True)
                n['leida'] = True # Se marcan como leídas al verlas
            if mis_notif and st.button("Limpiar historial"):
                st.session_state.notificaciones = [n for n in st.session_state.notificaciones if n not in mis_notif]
                st.rerun()
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            if st.form_submit_button("REGISTRAR EN SISTEMA", type="primary"):
                if f_id and f_cli and f_cor:
                    ...
                    # NOTIFICACIÓN ADMIN Y CLIENTE
                    agregar_notificacion(f"📦 Nuevo paquete registrado: {f_id} ({f_cli})", "admin")
                    agregar_notificacion(f"📦 Hemos recibido tu paquete {f_id} en almacén inicial.", "cliente", f_cor.lower().strip())
    ...
    with t_val:
        ...
        if pendientes:
            ...
            if st.button("CONFIRMAR VALIDACIÓN", type="primary"):
                ...
                # NOTIFICACIÓN CLIENTE: PESO VERIFICADO
                unidad = "ft³" if paq['Tipo_Traslado'] == "Marítimo" else "Kg"
                agregar_notificacion(f"✅ Peso/Medida verificada para {paq['ID_Barra']}: {valor_real} {unidad}", "cliente", paq['Correo'])
                
                guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()

    with t_cob:
        st.subheader("Gestión de Cobros")
        busq_cob = st.text_input("🔍 Buscar paquete para cobrar:", key="search_cob")
        pendientes_p = [p for p in st.session_state.inventario if p['Pago'] == 'PENDIENTE']
        if busq_cob: pendientes_p = [p for p in pendientes_p if busq_cob.lower() in p['ID_Barra'].lower() or busq_cob.lower() in p['Cliente'].lower()]
        
        for p in pendientes_p:
            total = float(p.get('Monto_USD', 0.0)); abo = float(p.get('Abonado', 0.0)); rest = total - abo
            with st.expander(f"💰 {p['ID_Barra']} — {p['Cliente']} (Resta: ${rest:.2f})"):
                ...
                if st.button(f"REGISTRAR PAGO", key=f"bp_{p['ID_Barra']}", type="primary"):
                    ...
                    agregar_notificacion(f"💰 Pago recibido: ${m_abono:.2f} para la guía {p['ID_Barra']}", "cliente", p['Correo'])
                    guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()
        ...
        if st.button("ACTUALIZAR ESTATUS", type="primary"):
            for p in st.session_state.inventario:
                if p["ID_Barra"] == sel_e: 
                    ...
                    agregar_notificacion(f"🚚 Tu guía {p['ID_Barra']} ahora está: {n_st}", "cliente", p['Correo'])
            guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()
        ...
        for est_k, est_l in [("RECIBIDO ALMACEN PRINCIPAL", "📦 EN ALMACÉN"), ("EN TRANSITO", "🚚 EN TRÁNSITO"), ("ENTREGADO", "✅ ENTREGADO")]:
            ...
            with st.expander(f"{est_l} ({len(df_f)})"):
                for _, r in df_f.iterrows():
                    st.markdown(f'<div class="resumen-row"><div style="color:#2563eb; font-weight:800;">{r["ID_Barra"]}</div><div>{r["Cliente"]}</div><div style="font-weight:700;">${float(r["Abonado"]):.2f}</div></div>', unsafe_allow_html=True)
...
def render_client_dashboard():
    ...
    if not mis_p: 
        st.info("No tienes envíos registrados con este correo.")
    else:
        c1, c2 = st.columns(2)
            ...
            with (c1 if i % 2 == 0 else c2):
                ...
                <div class="p-card">
                    ...
                    <div style="margin: 15px 0;">
                        <small style="opacity:0.7;">Total: <b>${tot:.2f}</b> | Abonado: <b>${abo:.2f}</b></small>
                    </div>
                    {"<div style='color:#facc15; font-size:12px;'>⚠️ Peso pendiente de validación</div>" if not p['Validado'] else "<div style='color:#4ade80; font-size:12px;'>✅ Peso verificado</div>"}
                </div>
                """, unsafe_allow_html=True)
