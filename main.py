...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_reg:
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            f_val_ini = st.number_input("Medida Inicial del Mensajero (Kg o ft³)", min_value=0.0, step=0.1)
    ...
    with t_val:
        st.subheader("⚖️ Validación y Control de Peso")
        ...
        if pendientes:
            ...
            st.info(f"**Cliente:** {paq['Cliente']} | **Traslado:** {tipo}")
            st.write(f"📏 Medida reportada inicialmente: **{paq['Peso_Mensajero']} {medida}**")
            
            v_real = st.number_input(f"Ingresar Medida Real en Almacén ({medida})", min_value=0.0, value=float(paq['Peso_Mensajero']), step=0.1)
            
            # Alarma por diferencia
            diferencia = abs(v_real - paq['Peso_Mensajero'])
            if diferencia > 0.01:
                st.markdown(f'<div class="warning-box">⚠️ ALERTA: Diferencia de {diferencia:.2f} {medida} detectada. El paquete pudo ser alterado.</div>', unsafe_allow_html=True)
            
            if st.button("Confirmar Validación y Recalcular"):
                paq['Peso_Almacen'] = v_real
                paq['Validado'] = True
                paq['Monto_USD'] = calcular_monto(v_real, tipo)
                guardar_datos(st.session_state.inventario, ARCHIVO_DB)
                st.success("✅ Validación completada. Monto actualizado.")
                st.rerun()
        else:
            st.info("No hay paquetes pendientes por validar medida.")
    ...
    with t_cob:
        ...
        for p in [p for p in st.session_state.inventario if p['Pago'] == 'PENDIENTE']:
            with st.expander(f"💰 {p['ID_Barra']} - {p['Cliente']}"):
                ...
                st.write(f"Total a pagar: **${p['Monto_USD']:.2f}** | Deuda: **${resta:.2f}**")
        ...
        if ver_p:
            ...
            if st.session_state.inventario:
                ...
                if paq_ed:
                    ...
                    with c3: n_med = st.number_input("Medida Validada", value=float(paq_ed['Peso_Almacen']))
...
# --- 5. PANEL DEL CLIENTE ---
    ...
    if not mis_p: st.info("Sin paquetes registrados.")
    else:
        for p in mis_p:
            with st.container():
                icon = "✈️" if p.get('Tipo_Traslado') == "Aéreo" else "🚢"
                total = p['Monto_USD']; abonado = p.get('Abonado', 0.0)
                st.markdown(f'<div class="p-card"><b>{icon} #{p["ID_Barra"]}</b><br>Estado: {p["Estado"]}<br>Falta pagar: ${(total-abonado):.2f}</div>', unsafe_allow_html=True)
                if total > 0: st.progress(min(abonado/total, 1.0))
