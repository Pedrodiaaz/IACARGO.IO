...
st.markdown("""
    ...
    .resumen-id { font-weight: 800; color: #2563eb; }
...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_reg:
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            f_pes = st.number_input("Peso Inicial Mensajero (Kg)", min_value=0.0, step=0.1)
    ...
    with t_val:
        st.subheader("Báscula de Almacén (Validación)")
        ...
        if pendientes:
            ...
            if st.button("⚖️ Validar Peso"):
                if abs(peso_real - paq['Peso_Mensajero']) > 0.5: st.warning("⚠️ Alerta: Variación de peso detectada.")
    ...
    with t_aud:
        ...
        if ver_p:
            ...
            if not df_aud.empty:
                ...
                if st.button("🗑️ Eliminar (Mover a Papelera)"):
        ...
        if st.session_state.inventario:
            ...
            for est_key, est_label in estados_mapeo.items():
                ...
                for _, row in df_f.iterrows():
                    ...
                    st.markdown(f'<div class="resumen-row"><div class="resumen-id">{icon} {row["ID_Barra"]}</div><div>{row["Cliente"]}</div><div>{row["Peso_Almacen"]:.1f} Kg | {row["Pago"]} | ${row["Abonado"]:.2f}</div></div>', unsafe_allow_html=True)

# --- 5. INTERFAZ DE CLIENTE ---
    ...
    if not mis_p:
        st.info("No hay paquetes registrados con tu correo.")
    else:
