...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_reg:
        st.subheader("Entrada de Paquete")
    ...
    with t_val:
        ...
        if pendientes:
            ...
            st.info(f"Cliente: {paq['Cliente']} | Peso Inicial: {paq['Peso_Mensajero']} Kg")
        ...
        if st.session_state.inventario:
            ...
            with c1:
                ...
                for idx, r in df_p.iterrows():
                    st.write(f"ID: {r['ID_Barra']} - ${r['Monto_USD']:.2f} ({r['Metodo_Pago']})")
    ...
    with t_est:
        st.subheader("Tránsito Logístico")
    ...
    with t_aud:
        ...
        if ver_p:
            ...
            if paq_ed:
                c1, c2 = st.columns(2)
                new_id = c1.text_input("ID", value=paq_ed['ID_Barra'])
                new_met = c2.selectbox("Método", ["Pago Inmediato", "Cobro Destino", "Pago en Cuotas"], index=0)
                b1, b2 = st.columns(2)
                with b1:
                    if st.button("💾 Guardar"):
                        paq_ed.update({'ID_Barra': new_id, 'Metodo_Pago': new_met})
                        guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()
                with b2:
    ...
    with t_res:
        ...
        if st.session_state.inventario:
            ...
            m3.metric("Recaudado", f"${df_res[df_res['Pago']=='PAGADO']['Monto_USD'].sum():.2f}")
            for est in ["RECIBIDO ALMACEN PRINCIPAL", "EN TRANSITO", "ENTREGADO"]:
                ...
                st.dataframe(df_f[['ID_Barra', 'Cliente', 'Monto_USD', 'Pago']], use_container_width=True, hide_index=True)
        ...
        for p in mis_p:
            ...
            st.markdown(f"""
                <div class="p-card">
                    ...
                    <p style="margin:5px 0;">Estado: <b>{p['Estado']}</b></p>
            ...
            if metodo == "Pago en Cuotas":
                st.markdown(f"""
                    <div style="background: rgba(0,128,255,0.05); padding: 15px; border-radius: 10px; margin-top:10px; border: 1px dashed #0080ff;">
                        <p style="margin:0; font-weight:bold; color:#1e3a8a;">Plan de Pagos:</p>
                        <table style="width:100%; font-size:13px; margin-top:5px;">
                            <tr><td>Inicial (30%):</td><td style="text-align:right;">${monto*0.3:.2f}</td></tr>
                            <tr><td>Cuota 1 (35%):</td><td style="text-align:right;">${monto*0.35:.2f}</td></tr>
                            <tr><td>Cuota 2 (35%):</td><td style="text-align:right;">${monto*0.35:.2f}</td></tr>
                        </table>
                    </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
